...
public:
    ...
    int info(Node* p) // Returns the content of the node
   {
    ...
    // Base case: if the node is NULL, it's a strictly binary subtree
    if (root == NULL) {
        return true;
    }

    // Check if the node is a leaf node (0 children)
    if (root->left == NULL && root->right == NULL) {
        return true;
    }

    // Check if the node has exactly 2 children
    if (root->left != NULL && root->right != NULL) {
        // Recursively check for the left and right subtrees
        return isStrictlyBinary(root->left) && isStrictlyBinary(root->right);
    }

    // If neither condition is satisfied, it's not a strictly binary tree
    return false;
}
